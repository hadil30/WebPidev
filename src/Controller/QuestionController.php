...
class QuestionController extends AbstractController
{
    ...
    #[Route('/user/admin/question/{quiz}', name: 'app_admin_question_show')]
    public function QuestionadminAction($quiz,QuestionRepository $questionRepository, EntityManagerInterface $entityManager,Request $request): Response
    {
        ...
        // Render the template with the form and Quizs data
        return $this->render('pages/admin/questionAdminlist.html.twig', [
            'questions' => $question,
            'quiz' => $quiz,
        ]);
    }
    ...
    $question->quiz= $entityManager->getRepository(Quiz::class)->find($request->get("quiz"));
        ...
        #[Route('/admin/questionadminAdd/{quiz}', name: 'app_admin_question_add')]
        public function QuestionadminAdd($quiz,Request $request, EntityManagerInterface $entityManager, SluggerInterface $slugger): Response
        {
            // Create a new Quiz instance
            $question = new Question();
        ...
        $question->quiz= $entityManager->getRepository(Quiz::class)->find($request->get("quiz"));
                ...
                return $this->redirectToRoute('app_admin_question_add', ['quiz' => $request->get("quiz")]);         
            }
        
            // Render the template with the form and Quiz data
            return $this->render('pages/admin/QuestionadminAdd.html.twig', [
                'form' => $form->createView(),
                'quizId' => $quiz,
            ]);}
    ...
    #[Route('/admin/questionadmindelete/{quiz}/{id}', name: 'question_admin_data_del')]

    public function deleteadmin($quiz,$id,EntityManagerInterface $entityManager, QuestionRepository $questionRepository): Response
    {
        $question=$questionRepository->find($id);
        ...
        $entityManager->flush();
        dump($question);
        return $this->redirectToRoute("app_admin_question_show",['quiz' => $quiz]);
    }
    ...
    #[Route('/user/quizadmin/{quiz}/{id}', name: 'question_admin_data_mod')]
    public function modifyadmin($quiz,$id, QuestionRepository $questionRepository, Request $request, EntityManagerInterface $entityManagerInterface): Response
    {
        $question = $questionRepository->find($id);
        $form = $this->createForm(QuestionType::class, $question);
        $form->handleRequest($request);
        ...
        return $this->render('pages/admin/QuestionadminMod.html.twig', [
            'form' => $form->createView(),
        ]);
    }
}
